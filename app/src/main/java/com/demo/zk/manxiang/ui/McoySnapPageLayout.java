...
public class McoySnapPageLayout extends ViewGroup {
	...
	public void getCurrentScrollView(){
		if(mViewPager==null)
			mViewPager = (CustomViewPager)mPageBottom.getRootView().findViewById(R.id.painter_service_pager);
		int currentItem = mViewPager.getCurrentItem();
		FragmentPagerAdapter fadapter = (FragmentPagerAdapter) mViewPager.getAdapter();
		Fragment item = (Fragment) fadapter.instantiateItem(mViewPager, currentItem);
		scrollView =(ViewGroup) (item.getView().findViewById(R.id.scroll_view));
	}
...
//		 DLog.i(TAG, "onLayout mDataIndex = " + mDataIndex);
	...
	private void addPagesAndRefresh() {
		...
		mPageBottom.getRootView().setId(0);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (yMoved) {
				...
				if( yDiff > 0 && mPageBottom.isAtTop() && mCurrentScreen == 1){
					getCurrentScrollView();
					boolean flag = false;
					if(scrollView instanceof ScrollView){
						ScrollView sv = (ScrollView)scrollView;
						flag = sv.getScrollY()==0;
					}
					if(scrollView instanceof RecyclerView){
						RecyclerView rv = (RecyclerView)scrollView;
						...
						flag = (count  == 0)||(c != null&&count >0&& c.getTop() == 0);
					}
					if(flag ){
						mTouchState = TOUCH_STATE_SCROLLING;
					}
