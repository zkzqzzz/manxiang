 ...
 */
public class ServiceRadioGroup extends GridLayout{
    ...
    private List<ServiceDetail.ServiceParam> params;
    ...
    public ServiceRadioGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.context = context;
        this.inflater = LayoutInflater.from(context);
    }

    public  void init( List<ServiceDetail.ServiceParam> params,final RadioGroupCallback callback){
        this.params = params;
        checkedIndex = -1;
        radioButtons.clear();
        this.removeAllViews();
        for(int i=0;i<params.size();i++) {
            ...
            radioButton .setText(params.get(i).getPaint_type_name());
    ...
    public ServiceDetail.ServiceParam get(){
        if(checkedIndex==-1)
            return  null;
